...
class EvmContractApiSpec extends Specification {
  ...
  {
    given:
    "An instance of EvmContractApi"
    def evmContractApi = new EvmContractApi(RESOURCE_PATH)

    when:
    "getDidDoc is called with a valid DID"
    def document = evmContractApi.getDidDoc("did:example:123456789abcdefghi?versionId=1")
    as DidDocument

    then:
    "The returned DidDocument has the expected context"
  ...
  private Proof newProof() {
    ...
    proof.proofValue = Hex.toHexString(signature.v) + Hex.toHexString(signature.r) +
